...
| `/alerts` | Створити одноразовий ціновий алерт, переглянути або скасувати алерти чату (`/allerts` також підтримується) |

### Цінові алерти

За ввімкненої функції `/alerts` відкриває вибір BTC, ETH, SOL або BNB та кнопки зростання/падіння на 5%, 10%, 20%.
Власний поріг: `/alerts BTC +7.5` або `/alerts ETH -12`. Підтримується до двох десяткових знаків:
зростання 0.01–1000%, падіння 0.01–99.99%. Пара котирування для всіх цих монет — USDT.

Базова ціна фіксується під час створення зі свіжого кешу (не старшого за одну хвилину), а не під час відкриття меню.
...
не може одразу активувати новий алерт. Кеш отримує час підтвердженого запису котирування в БД.

Алерти незалежні від регулярної підписки: `/unsubscribe` вимикає лише розсилку за інтервалом.
У списку `/alerts list` можна скасувати активний алерт або його повідомлення, яке ще очікує доставки.
Повідомлення, вже прийняте Telegram, не відкликається. У групі алерти спільні для чату: учасники можуть ними керувати.
Ліміт — 10 активних алертів разом із повідомленнями, які очікують доставки; список показує до 20 останніх записів,
надаючи перевагу активним. Однаковий активний поріг для тієї самої монети не дублюється.

Створення алерта, reply outbox і завершення inbox атомарні. Спрацювання та створення notification job також атомарні.
Унікальний job на алерт не є гарантією exactly-once Telegram delivery: при неоднозначному мережевому результаті
залишається обмеження зовнішнього API. Статуси доставки видно у списку; exhausted job не створюється заново.
Завершені алерти зберігаються 90 днів і видаляються лише після очищення пов'язаних notification jobs.
Активні алерти зберігаються до спрацювання або скасування.
Результати доставки: `cryptopulse_price_alert_deliveries_total{status}`; лічильники cron залишаються лише для регулярної розсилки.

Функція за замовчуванням вимкнена: `PRICE_ALERTS_ENABLED=false`. Після оновлення всіх replicas її можна ввімкнути.
Бот доповнить default menu командою `/alerts` (також для `uk`, `en`, `ru`), зберігши наявні команди.
Спеціальні chat scopes не змінюються; якщо вони налаштовані, додайте команду в них окремо.
Помилка синхронізації меню не зупиняє бота: команда та кнопки після `/start` продовжують працювати.
Порядок ввімкнення й відкату: [операційна інструкція](docs/operations.md#цінові-алерти).
